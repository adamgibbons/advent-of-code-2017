...
(define processList
  (lambda (lat accumulator)
    (cond
      ((null? lat) accumulator)
      ((null? (cdr lat)) accumulator)
       (else (cond 
               ((equal? (car lat) (cadr lat))
                (processList (cdr lat) (+ accumulator (car lat))))
                (else (processList (cdr lat) accumulator)))))))

(define processCapcha
  (lambda (string)
    (let ((lat (listNumbersFromString string)))
     (cond
       ((equal? (car lat) (car (reverse lat)))
        (processList lat (car lat)))
        (else (processList lat 0))))))
